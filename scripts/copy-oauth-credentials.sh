...
if [ -n "${OPENCLAW_INSTANCES_DIR:-}" ]; then
  ...
  # (e.g. fish), and `env` works the same across all of them. `-u` forces
  # unbuffered stdout so BEGIN/END markers stream in real time (and survive
  # a mid-run crash) instead of being held in the pipe buffer until exit.
  REMOTE_CMD="env OPENCLAW_INSTANCES_DIR_B64=$REMOTE_ENV_B64 python3 -u -"
fi
