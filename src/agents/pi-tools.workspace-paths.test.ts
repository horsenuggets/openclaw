...
// Track temp dirs for cleanup after all tests complete (avoids chdir race conditions)
...
describe.sequential("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    const workspaceDir = await makeTempDir("openclaw-ws-");
    const otherDir = await makeTempDir("openclaw-cwd-");
  ...
  it("lets exec workdir override the workspace default", async () => {
    const workspaceDir = await makeTempDir("openclaw-ws-");
    const overrideDir = await makeTempDir("openclaw-override-");
    const tools = createOpenClawCodingTools({ workspaceDir });
    const execTool = tools.find((tool) => tool.name === "exec");
    expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    const sandboxDir = await makeTempDir("openclaw-sandbox-");
    const workspaceDir = await makeTempDir("openclaw-workspace-");
    const sandbox = {
      enabled: true,
