...
- Agents: strip messaging-surface sections (`## Reply Tags`, `## Messaging`) from the `anthropic-subscription` system prompt. Their content (native reply/quote, cross-channel routing) diverges from the Claude Code identity and flipped subscription (OAuth) requests from free plan quota to paid extra usage — so per-channel agent requests were rejected once extra-usage balance hit zero. Verified by replaying prompts against the live API (dropping the section moved a spilling request back onto plan quota).
- CLI: add `openclaw auth mint-anthropic` to mint Claude (`anthropic-subscription`) OAuth tokens via the claude.ai flow and write them straight into an auth store (`--store shared` for the per-channel container store, `--store main`, or `--agent-dir <path>`), with no Keychain or `claude login` dependency so tokens can be minted directly on the deploy host. Supports two flows: a paste flow (prints the auth URL, prompts for the code Anthropic shows) and a localhost-callback flow (`--callback-port`) that captures the redirect automatically. `scripts/mint-anthropic-reauth.sh` drives the callback flow from a laptop over an SSH tunnel so the browser opens locally and tokens land on the deploy host with no copy-paste. The OAuth endpoints are overridable via `OPENCLAW_ANTHROPIC_AUTHORIZE_URL`/`OPENCLAW_ANTHROPIC_TOKEN_URL` for staging, and `scripts/e2e-mint-anthropic.sh` exercises the whole tunnel flow against a Docker container with a mock token endpoint.
...
- Discord router: render `/channel register`, `status`, and `unregister` as branded embeds (status is now a Property/Value table with a live agent-running probe), add an admin role (`OPENCLAW_ADMIN_ROLE_ID`) above the whitelist so admins can register any channel — including on another user's behalf via `/channel register owner:@user` — while whitelisted users may only register their own DM and unregister channels they own, and replace the text confirmation with a click-to-confirm button that only the user who opened the dialog can act on.
- Discord router: strip first-run onboarding control out of the router so onboarding is driven entirely by the agent's BOOTSTRAP.md rather than TypeScript. Removes the deterministic "post the Google connect card once USER.md has a name" trigger and all Google account-linking code (OAuth token exchange, gogcli import, the connect card, and the `send_hook_embed google` hook). The OAuth callback server is slimmed to its still-used purpose — the container-to-Discord send/read proxy — and renamed accordingly.
...
- Discord router: stop an endless error-reply loop where a channel whose agent kept failing (e.g. an instance provisioned without credentials, throwing `No API key found` / `invalid_grant`) would post "Something went wrong" on every gateway reconnect, all night. Message recovery now skips only genuine lifecycle banners (`*Back online.*` / `*Shutting down...*`) instead of every italic message, so an error reply correctly marks the user's message as already handled and is not re-attempted; and auth/config failures (missing key, expired or rotated OAuth token) are classified as admin problems and logged rather than echoed to the user as a misleading "please try again".
