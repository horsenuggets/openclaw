...
- CLI: add `openclaw auth mint-anthropic` to mint Claude (`anthropic-subscription`) OAuth tokens via the claude.ai flow and write them straight into an auth store (`--store shared` for the per-channel container store, `--store main`, or `--agent-dir <path>`). Runs anywhere over `ssh -t` with no browser callback, Keychain, or `claude login` dependency, so tokens can be refreshed directly on the deploy host.
