...
- Agents/Discord: fix agents losing memory in 1:1 DMs. Point instance provisioning at the container mount path (`/root/.openclaw`) so workspace/memory files are found, and sanitize copied configs in `openclawctl` so already-deployed channels get the same repair; teach the workspace instructions to load `MEMORY.md` in the owner's private 1:1 DM regardless of transport (not just web), while keeping the owner (not merely "any 1:1 DM") as the security boundary; and add `agents.defaults.skipBootstrapFile` to seed all starter files except `BOOTSTRAP.md` (and suppress an already-present stale `BOOTSTRAP.md` at load time) for router-managed instances that own onboarding.
